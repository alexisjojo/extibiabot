...
namespace exTibia.Helpers
{
    ...
    public struct PathFinderNode
    {
        ...
        private int _H;  // f = gone + heuristic
        ...
        public override bool Equals(object obj)
        {
            ...
            return Equals((PathFinderNode)obj);
        }
    ...
    public class PathFinder : IPathFinder
    {
        ...
        public Collection<Location> FindPath(Location start, Location end)
        {
            return FindPath(LocToPoint(start), LocToPoint(end));
        }

        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Performance", "CA1814:PreferJaggedArraysOverMultidimensional", MessageId = "Body")]
        public Collection<Location> FindPath(Point start, Point end)
        {
            ...
            while (mOpen.Count > 0 && !mStop)
            {
                ...
                for (int i = 0; i < (mDiagonals ? 8 : 4); i++)
                {
                    ...
                    if (mHeavyDiagonals && i > 3)
                        newG = parentNode.G + (int)(mGrid[newNode.X,newNode.Y] * 7.41);
                    else
            ...
            mCompletedTime = HighResolutionTime.Time();
            if (found)
            {
                ...
                foreach (PathFinderNode node in mClose)
                    result.Add(PointToLoc(node.X, node.Y));               
